      ...
      <div class="span10">
        ...
        <form action="salesreport.php" method="get">
          <center><strong>From : <input type="text" style="width: 223px; padding:14px;" name="d1" class="tcal"
                value="" /> To: <input type="text" style="width: 223px; padding:14px;" name="d2" class="tcal"
                value="" />
...
$d3 = $username;
$result = $db->prepare("SELECT * FROM sales WHERE cashier = :c AND date BETWEEN :a AND :b ORDER by transaction_id DESC ");
...
$d2 = date("Y-m-d", strtotime($_GET['d2']));
$d3 = $username;
$results = $db->prepare("SELECT sum(amount) FROM sales WHERE cashier = :c AND date BETWEEN :a AND :b");
...
?>
                ...
                <th colspan="1" style="border-top:1px solid #999999">
                  <?php
$resultia = $db->prepare("SELECT sum(profit) FROM sales WHERE cashier= :f AND date BETWEEN :d AND :e");
$resultia->bindParam(':d', $d1);
$resultia->bindParam(':e', $d2);
$resultia->bindParam(':f', $d3);
$resultia->execute();
